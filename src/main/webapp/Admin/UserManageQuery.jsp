...
function changePage(p){
	...
	location.href="<%=request.getContextPath()%>/UserListServlet.do?page="+p+"&size="+sizes;
}
function deleteByName(){
	var form=document.UserFrom;
	form.action="<%=request.getContextPath()%>/UserDeleteByNameServlet.do";
	//多选框效验
    ...
    <td width="835" background="<%=request.getContextPath()%>/images/wb_01 (3).jpg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <table width="553" border="1" align="center" cellpadding="0" cellspacing="1" bordercolor="#dadada" bgcolor="#FFFFFF">
          ...
          <c:forEach items="${userlist}" var="reuserBean" varStatus="userStatus">
            ...
            <input type="checkbox" name="reNames" value="${reuserBean.reNames}"></td>
            <td width="98" bordercolor="#FFFFFF"  class="text_cray1">${reuserBean.reNames}</td>
            <td width="80" bordercolor="#FFFFFF"  class="text_cray1">${reuserBean.sex}</td>
            <td width="132" bordercolor="#FFFFFF"  class="text_cray1">${reuserBean.cardTpye eq '1'?'二代身份证':(reuserBean.cardTpye eq 'C'?'港澳通行证':(reuserBean.cardTpye eq 'B'?'护照':'台湾通行证'))} </td>
            <td width="247" bordercolor="#FFFFFF"  class="text_cray1">${reuserBean.idNumber==null?'无':reuserBean.idNumber}</td>
            <td width="82" bordercolor="#FFFFFF"  class="text_cray1">${reuserBean.passengerType}</td>
            <td width="89" bordercolor="#FFFFFF"  class="text_cray1"><a href="<%=request.getContextPath()%>/UserByIdServlet.do?reNames=${reuserBean.reNames}" class="text_red">编辑</a></td>
          </tr>
          </c:forEach>
